...
func _input(event):
	if event.is_action_pressed("BasicAction") and player_in_chat_zone:
		$DialogueBigPete.start()
		is_chatting = true

func _on_big_pete_detect_body_entered(body):
	if body is Player:
		player_in_chat_zone = true
		var tween = get_tree().create_tween()
		tween.tween_property($"../../Player/Camera2D", "zoom", Vector2(1.7,1.7), 1)

func _on_big_pete_detect_body_exited(body):
	if body is Player:
		player_in_chat_zone = false
		var tween = get_tree().create_tween()
		tween.tween_property($"../../Player/Camera2D", "zoom", Vector2(1.5,1.5), 1)
		
func _on_dialogue_big_pete_dialogue_finished():
	is_chatting = false
